...
func (d *decodeState) decodeArray() error {
	...
	frameOffsetStart := bytesToInt(d.data[len(d.data)-d.frameOffsetSize:])
	...
	for frame := 0; frame < frameCount; frame++ {
		...
		endPosition := bytesToInt(frameBytes)
...
func (d *decodeState) decodeMap() error {
	...
	if !fixedWidthKey {
		keyBound := bytesToInt(d.data[len(d.data)-d.frameOffsetSize:])
