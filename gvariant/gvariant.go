...
func bytesToInt(offset []byte) uint64 {
	if len(offset) == 1 {
		return uint64(uint8(offset[0]))
	}
	if len(offset) == 2 {
		return uint64(binary.LittleEndian.Uint16(offset))
	}
	if len(offset) == 4 {
		return uint64(binary.LittleEndian.Uint32(offset))
	}
	if len(offset) == 8 {
		return uint64(binary.LittleEndian.Uint16(offset))
	}
...
func structIsVariant(v reflect.Value) bool {
	...
	if v.Type().PkgPath() != "github.com/chrisportman/go-gvariant/gvariant" {
		return false
	}
